...
// POST /api/venues/upload - Upload venue image
export async function POST(request: NextRequest) {
  try {
    ...
    // If venueId is provided, verify user owns the venue
    if (venueId) {
    ...
    const key = venueId 
      ? `venue-images/${userId}/${venueId}/${timestamp}-${randomStr}.webp`
      : `venue-images/${userId}/temp/${timestamp}-${randomStr}.webp`;
    ...
    // 2. Use sharp to resize to 200x200 and convert to WebP
    ...
    */
    ...
    console.error("Error in POST /api/venues/upload:", error);
    return NextResponse.json(
...
// DELETE /api/venues/upload - Delete venue image
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("Error in DELETE /api/venues/upload:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
