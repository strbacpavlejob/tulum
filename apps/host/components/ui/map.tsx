...
// ---------------------------------------------------------------------------
// MapMarkerClusterGroup – DOM-marker based clustering (like Leaflet's MarkerClusterGroup)
// ---------------------------------------------------------------------------

type MapMarkerClusterGroupProps = {
  /** MapMarker children to cluster */
  children: ReactNode;
  /** Custom render function for cluster icons. Receives the marker count. */
...
function DefaultClusterIcon({ count }: { count: number }) {
  ...
  return (
    <div
      className="flex items-center justify-center rounded-full bg-primary text-primary-foreground font-semibold shadow-md border-2 border-background"
...
function MapMarkerClusterGroup({
  ...
  const [clusters, setClusters] = useState<
    Supercluster.ClusterFeature<{ index: number }>[]
  >([]);
  const [visibleLeaves, setVisibleLeaves] = useState<Set<number>>(new Set());

  // Extract MapMarker children and their coordinates
  const markers = useMemo(() => {
    ...
    Children.forEach(children, (child) => {
      if (
        ...
        child.props !== null &&
        "longitude" in child.props &&
        "latitude" in child.props
      ) {
        result.push({
          longitude: child.props.longitude as number,
          latitude: child.props.latitude as number,
          element: child,
        });
      }
    });
  ...
  // Update visible clusters/leaves on map move
  useEffect(() => {
    ...
    const update = () => {
      ...
      const clusterFeatures: Supercluster.ClusterFeature<{
        index: number;
      }>[] = [];
      ...
      for (const f of features) {
        if ("cluster" in f.properties && f.properties.cluster) {
          clusterFeatures.push(
            f as Supercluster.ClusterFeature<{ index: number }>,
          );
        } else {
          leafIndices.add(
            (f as Supercluster.PointFeature<{ index: number }>).properties
              .index,
          );
        }
      }
    ...
    <>
      ...
      {clusters.map((cluster) => {
        ...
        return (
          ...
          >
            <MarkerContent>
              {icon ? icon(count) : <DefaultClusterIcon count={count} />}
            </MarkerContent>
          </MapMarker>
        );
      })}

      {/* Render unclustered child markers */}
      {markers.map(
        (m, i) =>
          visibleLeaves.has(i) && <Fragment key={i}>{m.element}</Fragment>,
      )}
    </>
  );
}
